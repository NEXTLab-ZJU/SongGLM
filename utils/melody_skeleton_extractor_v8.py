...
class Melody_Skeleton_Extractor:
    ...
    def __init__(self, midi, resolution=480, grids=16):
        ...
        self.subsections,self.notes = self._divide_subsections()
    ...
    def _get_long(self):
        ...
        for bar_id, bar_notes in self.subsections.items():
            ...
            # 根据最长音符的数量选择适当的索引
            # 当 max_duration_count 等于1时，正常。。。
            # 当 ……大于1小于len（duration list）时，只取第一个出现的，作为长音
            # 当小节内音符duration都相等时，不取长音

            if max_duration_count == 1 or (1 < max_duration_count < len(duration_list)):
                idx_list = [i for i, duration in enumerate(duration_list) if duration == max_duration]
                if max_duration_count > 1:
                    idx_list = idx_list[:1]  # 只取第一个最长音符
            else:
                idx_list = []
